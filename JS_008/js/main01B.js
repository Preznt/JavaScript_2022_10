...
document.addEventListener("DOMContentLoaded", () => {
   ...
   *    학번, 이름 : 입력이 되지 않았을 때 alert 띄우고 다시입력
   ...
   * 6. 다시(새로고침) 시작했을때 localStorage에 보관된 데이터를
   *        읽어와서 table에 보이기
   */
  // 유효성 검사
  const scoreCheck = () => {
    ...
    for (let i = 0; i < inputScores.length; i++) {
      ...
      /**
       * for()반목분의 의해서 점수를 입력받는 input 만큼
       ...
       *   검사 과정을 즉시 중단한다
       *      그리고 return false 로 false값을 되돌려준다
       */
      if (!input.value) {
        // 1
      ...
      // 입력된 box 문자열ㅇ르 clear 하여
      // 다시 입력을 쉽게 할 수 있도록 도와주는 역할
      // input.value = "";
  ...
  const scoreInput = () => {
    ...
    /**
     * 한 사람의 성적에는 5개의 항목(학번, 이름, 국어, 영어, 수학) 있다
  ...
  const scorePrint = () => {
    ...
    // table 의 tbody 에 이미 추가된 모든 tr, td요소 제거
    ...
    for (let i = 0; i < scores.length; i++) {
       ...
       *  scores = [
       *    {num:"1", name: "홍길동", kor:90, eng:80, math:70, sc_total: 240, sc.avg: 80},
       *    {num:"2", name: "홍길동", kor:90, eng:80, math:70, sc_total: 240, sc.avg: 80}
  ...
  btnOk?.addEventListener("click", () => {
     ...
     * 완료되면 true 를 return하고,
     * 그렇지 않으면 false를  return 할 것이다 라는것을
     * 미리 예상하고 작성하는 코드이다
     */
    if (scoreCheck()) {
      scoreInput();
